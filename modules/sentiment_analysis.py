...
class SentimentAnalyzer:
    ...
    def get_news_sentiment(self, symbol):
        try:
            ...
            response = requests.get(url, params=params, headers=headers, proxies=proxies, timeout=self.api_timeout)
            if response.status_code == 200:
                ...
                logger.log(f"[SENTIMENT] News API başarısız: {response.status_code}")
                return 50
    ...
    def get_market_sentiment(self, symbol):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.log(f"[SENTIMENT] Binance API başarısız: {response.status_code}")
                return 50
        except Exception as e:
